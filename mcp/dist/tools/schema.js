...
export function registerSchemaTools(server, client) {
    server.tool("create_object", "Create a new custom object type definition (e.g. 'Project', 'Deal'). Defines the schema for a new entity type in your workspace.", {
    ...
    }, { readOnlyHint: false }, async ({ name, slug, name_plural, description, type }) => {
    ...
    server.tool("get_object", "Get a single object type definition with its attributes. Use this to discover available fields before creating or querying records.", { slug: z.string().describe("Object type slug (e.g. 'person', 'company', 'deal')") }, { readOnlyHint: true }, async ({ slug }) => {
        const result = await client.get(`/v1/objects/${encodeURIComponent(slug)}`);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("list_objects", "List all object type definitions in the workspace. Call this first to discover available object types and their schemas before creating or querying records.", {
        include_attributes: z.boolean().optional().describe("Include attribute definitions in the response"),
    }, { readOnlyHint: true }, async ({ include_attributes }) => {
    ...
    server.tool("update_object", "Update an existing object type definition (name, description, plural name).", {
    ...
    }, { readOnlyHint: false }, async ({ slug, name, name_plural, description }) => {
        const body = {};
        if (name !== undefined)
            body.name = name;
        if (name_plural !== undefined)
    ...
    server.tool("delete_object", "Delete an object type definition and ALL its records. This is destructive and cannot be undone.", { slug: z.string().describe("Object type slug to delete") }, { readOnlyHint: false, destructiveHint: true }, async ({ slug }) => {
        const result = await client.delete(`/v1/objects/${encodeURIComponent(slug)}`);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("create_attribute", "Add a new attribute (field) to an object type. Supports types: text, number, email, phone, url, date, boolean, currency, location, select, social_profile, domain, full_name.", {
    ...
    }, { readOnlyHint: false }, async ({ object_slug, name, slug, type, description, options }) => {
    ...
    server.tool("update_attribute", "Update an existing attribute definition on an object type.", {
    ...
    }, { readOnlyHint: false }, async ({ object_slug, attribute_id, name, description, options }) => {
        const body = {};
        if (name !== undefined)
            body.name = name;
        if (description !== undefined)
    ...
    server.tool("delete_attribute", "Delete an attribute from an object type. This removes the field and its data from all records. Cannot be undone.", {
        object_slug: z.string().describe("Object type slug"),
        attribute_id: z.string().describe("Attribute ID to delete"),
    }, { readOnlyHint: false, destructiveHint: true }, async ({ object_slug, attribute_id }) => {
