...
export function registerRelationshipTools(server, client) {
    server.tool("create_relationship_definition", "Define a new relationship type between two object types (e.g. person 'works at' company).", {
    ...
    }, { readOnlyHint: false }, async ({ type, entity_definition_1_id, entity_definition_2_id, entity_1_to_2_predicate, entity_2_to_1_predicate }) => {
    ...
    server.tool("list_relationship_definitions", "List all relationship type definitions in the workspace.", {}, { readOnlyHint: true }, async () => {
        const result = await client.get("/v1/relationships");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("delete_relationship_definition", "Delete a relationship type definition. This removes all instances of this relationship. Cannot be undone.", { id: z.string().describe("Relationship definition ID to delete") }, { readOnlyHint: false, destructiveHint: true }, async ({ id }) => {
        const result = await client.delete(`/v1/relationships/${encodeURIComponent(id)}`);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("create_relationship", "Link two records using an existing relationship definition.", {
    ...
    }, { readOnlyHint: false }, async ({ record_id, definition_id, entity_1_id, entity_2_id }) => {
    ...
    server.tool("delete_relationship", "Remove a relationship between two records. Cannot be undone.", {
        record_id: z.string().describe("Record ID"),
        relationship_id: z.string().describe("Relationship instance ID to delete"),
    }, { readOnlyHint: false, destructiveHint: true }, async ({ record_id, relationship_id }) => {
